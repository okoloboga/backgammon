...
2.  Эти последовательности учитывают все правила игры:
    ...
    *   Приоритет большего хода, если можно использовать только одну кость.
...
Сервер не отправляет отдельных событий. Вместо этого клиент подписывается на `room.onStateChange((newState) => { ... })` и реагирует на любые изменения в `GameState`, такие как:
- Появление `dice` (результат броска).
- Изменение `currentPlayer`.
- Обновление `possibleMoves`.
- Изменение `board`, `bar`, `off`.
- Появление `winner`.
