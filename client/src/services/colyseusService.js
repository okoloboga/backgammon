...
class ColyseusService {
   ...
   * @param {function(colyseus.RoomAvailable[])} onRoomsUpdate - Колбэк, вызываемый с полным списком комнат.
   * @param {function(string, colyseus.RoomAvailable)} onRoomAdded - Колбэк при добавлении комнаты.
   * @param {function(string)} onRoomRemoved - Колбэк при удалении комнаты.
   * @returns {function()} - Функция для отписки от всех событий.
   */
  onRoomsChange(onRoomsUpdate, onRoomAdded, onRoomRemoved) {
    if (!this.lobby) {
    ...
    const handleAdd = (roomId, room) => onRoomAdded(roomId, room);
