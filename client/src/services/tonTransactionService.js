...
class TonTransactionService {
  ...
  /**
   * Get ClaimJetton payload from backend
   * @param {bigint} gameId - Game ID to claim
   * @returns {Promise<{payload: string, escrowAddress: string}>}
   */
  async getClaimJettonPayload(gameId) {
    const response = await fetch(`${API_BASE_URL}/game-http/build-claim-payload`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ gameId: gameId.toString() }),
    });
  ...
  /**
   * Claim RUBLE (Jetton) winnings for winner
   * @param {bigint} gameId - Game ID to claim
   * @returns {Promise<{success: boolean, txHash?: string, error?: string}>}
   */
  async claimJettonWinnings(gameId) {
    if (this.useMockTransactions) {
      console.log(`[MOCK] ClaimJetton: gameId=${gameId}`);
      return { success: true, txHash: `mock_claim_${gameId}` };
    }
    ...
    try {
      // Get payload from backend
      const { payload, escrowAddress } = await this.getClaimJettonPayload(gameId);
      ...
      const transaction = {
        ...
        messages: [
          {
            address: escrowAddress,
            amount: GAS_AMOUNT, // Only gas needed for claim
            payload: payload,
          },
        ],
      };
      ...
      if (result.boc) {
        console.log(`ClaimJetton sent: boc=${result.boc.substring(0, 50)}...`);
        return { success: true, boc: result.boc, txHash: result.boc };
      }

      return { success: false, error: 'Transaction rejected' };
    } catch (error) {
      console.error('ClaimJetton failed:', error);
      return { success: false, error: error.message || 'Transaction failed' };
    }
  }
