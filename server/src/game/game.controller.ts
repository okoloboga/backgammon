...
async matchmake( @Body() options: MatchmakeDto): Promise<{
  ...
  @Post('build-claim-payload')
  @ApiOperation({ summary: 'Build ClaimJetton payload for winner to claim RUBLE' })
  @ApiResponse({
    status: 200,
    description: 'Base64-encoded BOC payload',
    schema: {
  ...
  buildClaimPayload(
    ...
    const payload = this.escrowService.buildClaimJettonPayload(BigInt(dto.gameId));
    return {
      payload,
      escrowAddress: this.escrowService.getContractAddress(),
    };
